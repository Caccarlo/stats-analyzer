...
Opening `Previsioni` sends the already-known match snapshot to the local server. The forecast then uses only SQLite rows whose timestamp is before kickoff. It does not request SofaScore statistics for any historical match. SofaScore remains the source for navigation, lineups, logos, match history, and other non-model UI.
